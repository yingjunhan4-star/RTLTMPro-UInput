...
namespace UnityEngine.UI
{
    ...
    [AddComponentMenu("UI/RTL Input Field", 103)]
    public class RTLInputField
        : Selectable,
        ...
        ILayoutElement
    {
        ...
        [SerializeField] private bool m_Farsi = true;
        [SerializeField] private bool m_PreserveNumbers;
        [SerializeField] private bool m_ForceFix;
        private RTLInputFieldVisualText m_VisualText = RTLInputFieldVisualText.Identity(string.Empty);
        ...
        public bool farsi
        {
            ...
            set
            {
                if (SetPropertyUtility.SetStruct(ref m_Farsi, value))
                    UpdateLabel();
            }
        }
        ...
        public bool preserveNumbers
        {
            ...
            set
            {
                if (SetPropertyUtility.SetStruct(ref m_PreserveNumbers, value))
                    UpdateLabel();
            }
        }
        ...
        {
            get { return m_ForceFix; }
            set
            {
                if (SetPropertyUtility.SetStruct(ref m_ForceFix, value))
                    UpdateLabel();
            }
        }
        ...
        /// <summary>
        /// This is an optional 闂傚倸鍊搁崐鎼佸磹閻戣姤鍤勯柛顐ｆ礀绾惧鏌曟繛鐐珔缁炬儳娼￠弻锛勪沪鐠囨彃濮岄梺閫炲苯澧柟铏姉缁辨捇骞樼拠鑼幐闂佽崵鍠庨崜姘舵⒒?graphic to show that the InputField text field is empty. Note that this 闂傚倸鍊搁崐鎼佸磹閻戣姤鍤勯柛顐ｆ礀绾惧鏌曟繛鐐珔缁炬儳娼￠弻锛勪沪鐠囨彃濮岄梺閫炲苯澧柟铏姉缁辨捇骞樼拠鑼幐闂佽崵鍠庨崜? graphic still displays even when the InputField is selected (that is; when there is focus on it).
        ...
        ///     void Start()
        ///     {
        ///         // changes the password symbol. 0 = $, 1 = ! 2 = 闂?and so on.
        ///         mainInputField.asteriskChar = "$!闂?&*"[0];
        ///     }
        /// }
        /// ]]>
        ///</code>
        ...
        private void MoveLineStart(bool shift)
        {
            int position = GetLineLogicalStart(caretSelectPositionInternal);
        ...
        private void MoveLineEnd(bool shift)
        {
            int position = GetLineLogicalEnd(caretSelectPositionInternal);
        ...
        protected int GetCharacterIndexFromPosition(Vector2 pos)
        {
            ...
            if (line >= gen.lineCount)
                return ToLogicalIndex(VisibleToVisualIndex(gen.characterCountVisible));
            ...
            for (int i = startCharIndex; i < endCharIndex; i++)
            {
                ...
                if (distToCharStart < distToCharEnd)
                    return ToLogicalIndex(VisibleToVisualIndex(i));
            }

            return ToLogicalIndex(VisibleToVisualIndex(endCharIndex));
        }
        ...
        public virtual void OnDrag(PointerEventData eventData)
        {
            ...
            caretSelectPositionInternal = GetCharacterIndexFromPosition(localMousePos);
        ...
        public override void OnPointerDown(PointerEventData eventData)
        {
            ...
            if (hadFocusBefore)
            {
                ...
                caretSelectPositionInternal = caretPositionInternal = GetCharacterIndexFromPosition(localMousePos);
            }
        ...
        {
            int visualPos = ToVisualIndex(originalPos);
            if (visualPos >= cachedInputTextGenerator.characters.Count)
                return 0;

            UICharInfo originChar = cachedInputTextGenerator.characters[visualPos];
            int originLine = DetermineCharacterLine(visualPos, cachedInputTextGenerator);
            ...
            for (int i = cachedInputTextGenerator.lines[originLine - 1].startCharIdx; i < endCharIdx; ++i)
            {
                if (cachedInputTextGenerator.characters[i].cursorPos.x >= originChar.cursorPos.x)
                    return ToLogicalIndex(i);
            }
            return ToLogicalIndex(endCharIdx);
        }
        ...
        {
            int visualPos = ToVisualIndex(originalPos);
            if (visualPos >= cachedInputTextGenerator.characterCountVisible)
                return text.Length;
        ...
        protected void UpdateLabel()
        {
            if (m_TextComponent != null && m_TextComponent.font != null && !m_PreventFontCallback)
            {
                ...
                string processed = inputType == InputType.Password ? new string(asteriskChar, fullText.Length) : fullText;
                m_VisualText = inputType == InputType.Password
                    ? RTLInputFieldVisualText.Identity(processed)
                    : RTLInputFieldTextProcessor.Build(processed, m_Farsi, m_PreserveNumbers, m_ForceFix);
                processed = m_VisualText.Text;
                ...
                if (!isEmpty)
                {
                    ...
                    SetDrawRangeToContainCaretPosition(ToVisualIndex(caretSelectPositionInternal));
        ...
        private int ToVisualIndex(int logicalIndex)
        {
            return m_VisualText.ToVisualIndex(logicalIndex);
        }

        private int ToLogicalIndex(int visualIndex)
        {
            return m_VisualText.ToLogicalIndex(visualIndex);
        }

        private int VisibleToVisualIndex(int visibleIndex)
        {
            return Mathf.Clamp(visibleIndex + m_DrawStart, 0, m_VisualText.Text.Length);
        }

        private bool IsSelectionVisible()
        {
            if (m_DrawStart > ToVisualIndex(caretPositionInternal) || m_DrawStart > ToVisualIndex(caretSelectPositionInternal))
                return false;

            if (m_DrawEnd < ToVisualIndex(caretPositionInternal) || m_DrawEnd < ToVisualIndex(caretSelectPositionInternal))
                return false;
        ...
        private int GetLineLogicalStart(int logicalPos)
        {
            if (cachedInputTextGenerator.lineCount <= 0)
                return 0;

            int visualPos = Mathf.Clamp(ToVisualIndex(logicalPos), 0, cachedInputTextGenerator.characterCountVisible);
            int line = DetermineCharacterLine(visualPos, cachedInputTextGenerator);
            int visualLineStart = GetLineStartPosition(cachedInputTextGenerator, line);
            return ToLogicalIndex(visualLineStart);
        }

        private int GetLineLogicalEnd(int logicalPos)
        {
            if (cachedInputTextGenerator.lineCount <= 0)
                return text.Length;
            ...
            int visualPos = Mathf.Clamp(ToVisualIndex(logicalPos), 0, cachedInputTextGenerator.characterCountVisible);
            int line = DetermineCharacterLine(visualPos, cachedInputTextGenerator);
            int visualLineEnd = GetLineEndPosition(cachedInputTextGenerator, line);
            int visualBoundary = visualLineEnd < cachedInputTextGenerator.characterCountVisible ? visualLineEnd + 1 : visualLineEnd;
            return ToLogicalIndex(visualBoundary);
        }

        private float GetCursorPositionX(TextGenerator gen, int charPos)
        {
            if (gen == null || gen.characterCountVisible <= 0)
            {
                return m_VisualText.IsRightToLeft || m_Farsi || m_ForceFix
                    ? m_TextComponent.rectTransform.rect.xMax
                    : m_TextComponent.rectTransform.rect.xMin;
            }

            charPos = Mathf.Clamp(charPos, 0, gen.characterCountVisible);

            if (charPos < gen.characters.Count)
                return gen.characters[charPos].cursorPos.x / m_TextComponent.pixelsPerUnit;

            UICharInfo previousChar = gen.characters[gen.characterCountVisible - 1];
            return (previousChar.cursorPos.x + previousChar.charWidth) / m_TextComponent.pixelsPerUnit;
        }
        ...
        private void GenerateCaret(VertexHelper vbo, Vector2 roundingOffset)
        {
            ...
            int adjustedPos = Mathf.Max(0, ToVisualIndex(caretPositionInternal) - m_DrawStart);
            TextGenerator gen = m_TextComponent.cachedTextGenerator;
            ...
            startPosition.x = GetCursorPositionX(gen, adjustedPos);
            ...
            else if (startPosition.x < m_TextComponent.rectTransform.rect.xMin)
        ...
        {
            int startChar = Mathf.Max(0, ToVisualIndex(caretPositionInternal) - m_DrawStart);
            int endChar = Mathf.Max(0, ToVisualIndex(caretSelectPositionInternal) - m_DrawStart);
            ...
            while (currentChar <= endChar && currentChar < gen.characterCount)
            {
                ...
                {
                    float startX = GetCursorPositionX(gen, startChar);
                    float endX = GetCursorPositionX(gen, currentChar + 1);
                    float leftX = Mathf.Min(startX, endX);
                    float rightX = Mathf.Max(startX, endX);
                    Vector2 startPosition = new Vector2(leftX, gen.lines[currentLineIndex].topY / m_TextComponent.pixelsPerUnit);
                    Vector2 endPosition = new Vector2(rightX, startPosition.y - gen.lines[currentLineIndex].height / m_TextComponent.pixelsPerUnit);

                    if (endPosition.x > m_TextComponent.rectTransform.rect.xMax)
                        endPosition.x = m_TextComponent.rectTransform.rect.xMax;
                    if (startPosition.x < m_TextComponent.rectTransform.rect.xMin)
                        startPosition.x = m_TextComponent.rectTransform.rect.xMin;
